...
def is_readonly_mode() -> bool:
    """当前进程是否以只读模式访问数据库（由环境变量 DB_READONLY=1 开启）。

    Docker 部署中，streamlit 容器设置 DB_READONLY=1，
    避免与 daemon 容器的写连接冲突。
    """
    return os.environ.get("DB_READONLY", "").strip() in ("1", "true", "yes")


def _open(db_path: str, readonly: bool = False) -> duckdb.DuckDBPyConnection:
    """打开 DuckDB 连接（短连接，调用方负责关闭）。

    DuckDB 不允许同一进程对同一文件同时存在 read_only=True 和
    read_only=False 的连接。只读模式下所有连接统一 read_only=True。
    """
    # 只读模式下强制所有连接为 read_only=True，避免同进程混用报错
    if is_readonly_mode():
        readonly = True
...
def get_read_connection(db_path: str = _DEFAULT_DB_PATH) -> duckdb.DuckDBPyConnection:
    ...
    DuckDB 不允许同一进程对同一文件同时存在 read_only=True 和
    read_only=False 的连接，否则报 "different configuration" 错误。
    因此：非只读模式下统一返回 read_write 连接，避免混用；
          只读模式下所有连接统一 read_only=True。
    """
...
CREATE TABLE IF NOT EXISTS trade_calendar (
    ...
    is_trading  BOOLEAN NOT NULL      -- 1=交易日 0=非交易日
);
"""
...
def _ensure_tables(conn: duckdb.DuckDBPyConnection) -> None:
    """建立五张核心表（如已存在则跳过）。"""
    for ddl in (_DDL_STOCK_INFO, _DDL_DAILY_BAR, _DDL_MINUTE_BAR, _DDL_INDEX_DAILY_BAR, _DDL_TRADE_CALENDAR):
        conn.execute(ddl)
...
def upsert_trade_calendar(df: pd.DataFrame, db_path: str = _DEFAULT_DB_PATH) -> int:
    """
    写入交易日历（UPSERT）。

    参数:
        df: 含 ["date", "is_trading"] 列的 DataFrame
            is_trading 应为 0/1 或 True/False
        db_path: 数据库路径
    ...
    df["is_trading"] = df["is_trading"].astype(bool)
    conn = get_connection(db_path)
    try:
        conn.execute(_DDL_TRADE_CALENDAR)
        total = 0
        for _, row in df.iterrows():
            conn.execute(
                """INSERT OR REPLACE INTO trade_calendar (date, is_trading)
                   VALUES (?, ?)""",
                [row["date"], row["is_trading"]],
            )
            total += 1
        logger.info(f"trade_calendar: 写入 {total} 条")
        return total
    finally:
        conn.close()
...
def get_trade_dates(
    ...
    参数:
        start_date: "YYYY-MM-DD"，None 表示不限
        end_date:   "YYYY-MM-DD"，None 表示不限

    返回:
        交易日日期字符串列表，如 ["2025-01-02", "2025-01-03", ...]
    """
    conn = get_read_connection(db_path)
    try:
        sql = "SELECT date FROM trade_calendar WHERE is_trading = true"
        params = []
        if start_date:
            sql += " AND date >= ?"
            params.append(start_date)
...
def is_trade_date(date: str, db_path: str = _DEFAULT_DB_PATH) -> bool:
    """判断某日期是否为交易日。"""
    conn = get_read_connection(db_path)
    try:
        r = conn.execute(
            "SELECT is_trading FROM trade_calendar WHERE date = ?", [date]
        ).fetchone()
        return r is not None and r[0] is True
    finally:
        conn.close()
...
def find_nearest_trade_date(
    ...
    """
    找到离指定日期最近的交易日。
    若 trade_calendar 表为空，自动从 BaoStock 拉取近 2 年日历并写入，再重试。

    direction:
        "backward" — 往前找最近的交易日（含当天）
        "forward"  — 往后找最近的交易日（含当天）
    """
    conn = get_read_connection(db_path)
    try:
        # 先检查表是否有数据，若无则自动补拉
        cnt = conn.execute("SELECT COUNT(*) FROM trade_calendar").fetchone()[0]
        if cnt == 0:
            conn.close()
            _auto_fill_trade_calendar(db_path)
            conn = get_read_connection(db_path)

        if direction == "backward":
            r = conn.execute(
                "SELECT date FROM trade_calendar WHERE date <= ? AND is_trading = true ORDER BY date DESC LIMIT 1",
                [date],
            ).fetchone()
        else:
            r = conn.execute(
                "SELECT date FROM trade_calendar WHERE date >= ? AND is_trading = true ORDER BY date ASC LIMIT 1",
                [date],
            ).fetchone()
...
def _auto_fill_trade_calendar(db_path: str = _DEFAULT_DB_PATH) -> None:
    """自动从 BaoStock 拉取近 2 年交易日历并写入（无感补拉）"""
    try:
        from data_tools import fetch_trade_dates
        today = date.today()
        start = (today.replace(year=today.year - 2)).strftime("%Y-%m-%d")
        end = today.strftime("%Y-%m-%d")
        cal_df = fetch_trade_dates(start, end)
        if cal_df.empty:
            return
        ...
        upsert_trade_calendar(cal_df, db_path)
        logger.info(f"_auto_fill_trade_calendar: 自动写入 {len(cal_df)} 条日历数据")
    except Exception as e:
        logger.warning(f"_auto_fill_trade_calendar 失败（非致命）: {e}")
    ...
    try:
        ...
        for tbl, date_col in [
            ...
            try:
                ...
                rows.append({"table": tbl, "rows": 0, "min_date": None, "max_date": None})
                continue
            if date_col:
                r = conn.execute(
...
def check_daily_integrity(
    ...
    """
    检查日线数据完整性：找出在 stock_info 中但某日无 daily_bar 数据的股票。

    逻辑：
      1. 确定 target_date（默认取 daily_bar 最大日期）
      2. stock_info 中所有在市股票作为基准
      3. 找出该日期在 daily_bar 中无数据的股票
      4. 排除当天停牌/未上市的股票（上市日期 > target_date）

    参数:
        target_date: 检查日期，为 None 时自动取 daily_bar 最新日期
        adjustflag:  复权方式，默认 '3'（不复权）
        db_path:     数据库路径
    ...
    返回:
        pd.DataFrame，列::
            ...
            code_name   股票名称
            ipo_date    上市日期
            reason      缺失原因（上市未拉取 / 未知缺失）

    使用示例::

        missing = check_daily_integrity()
        print(f"缺失 {len(missing)} 只股票")
    """
    conn = get_read_connection(db_path)
    try:
        # 1. 确定 target_date
        if target_date is None:
            target_date = conn.execute(
                f"SELECT MAX(date) FROM daily_bar WHERE adjustflag = '{adjustflag}'"
            ).fetchone()[0]
            if target_date is None:
                logger.warning("check_daily_integrity: daily_bar 无数据")
                return pd.DataFrame(columns=["code", "code_name", "ipo_date", "reason"])

        target_date = str(target_date)
        logger.info(f"检查日线完整性: date={target_date}, adjustflag={adjustflag}")

        # 2. 找出 stock_info 中有但 daily_bar 中该日无数据的股票
        missing_df = conn.execute(
            """
            SELECT s.code, s.code_name, s.ipo_date,
                CASE WHEN s.ipo_date > ? THEN '上市未拉取' ELSE '未知缺失' END as reason
            FROM stock_info s
            LEFT JOIN (
                SELECT DISTINCT code FROM daily_bar
                WHERE date = ? AND adjustflag = ?
            ) d ON s.code = d.code
            WHERE d.code IS NULL
              AND s.type = '1'
              AND (s.out_date IS NULL OR s.out_date > ?)
            ORDER BY s.code
            """,
            [target_date, target_date, adjustflag, target_date],
        ).df()

        total = conn.execute(
            "SELECT COUNT(*) FROM stock_info WHERE type = '1' AND (out_date IS NULL OR out_date > ?)",
            [target_date],
        ).fetchone()[0]

        logger.info(
            f"完整性检查完成: 在市 {total} 只, 缺失 {len(missing_df)} 只 "
            f"({len(missing_df) / total * 100:.1f}%)"
        )
        return missing_df
